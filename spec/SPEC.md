...
- 8.1 **Constant-time comparison.** Tag comparison MUST use a fixed-time primitive: .NET `CryptographicOperations.FixedTimeEquals` (net8.0) / structural XOR-accumulate polyfill (netstandard2.0; no early return, full-length loop, `NoInlining|NoOptimization`); Node `crypto.timingSafeEqual`; Python `hmac.compare_digest`; PHP `hash_equals`. On the Edge/WASM platform, where no runtime-provided constant-time primitive is available, the pure-JS implementation uses `Uint32Array` XOR-accumulation with a dead-code-elimination guard (`(diff|0)===0`) and no early return. Implementations MUST document that this is not guaranteed constant-time under V8/workerd JIT; the residual timing channel is acceptable in practice because the fixed 32-byte tag comparison is dwarfed by the hundreds-of-milliseconds Argon2id computation (ADR 0006).
...
- 8.8 **Edge/WASM memory-limit environment.** On WASM engines, argon2id places `m` KiB of blocks into WASM linear memory. In a memory-constrained runtime (notably the Cloudflare Workers 128 MiB isolate, whose WASM page budget is 2048 × 64 KiB), a profile or externally produced hash whose `m` exceeds the page budget cannot be computed. The Edge implementation MUST enforce a configurable page cap (default 2048, raisable via `maxWasmPages` for higher-memory runtimes such as browsers) and raise `UnsupportedEnvironment` (`environment.argon2id_unavailable`) — a typed error, never an opaque isolate OOM — both when hashing at such a profile and when verifying such a hash (preserves the V1 single-meaning rule). Concretely, `highest` (m=131072 KiB) needs 2049 pages and is therefore unusable on default-capped Workers; `high` (64 MiB) is the recommended maximum there. This is an environment capability limit, not a spec parameter change — the frozen profiles (§3) are unchanged, and every other platform computes them normally.
