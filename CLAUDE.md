...
3. `matrix.yml`（守門 3，擋 merge）：`spec/tools/matrix.py` 跑 **5×5** round-trip（25 platform-pair × 凍結密碼集 × 三檔位）＋五平台 harness 契約
...
python3 spec/tools/matrix.py                                # 守門 3：5×5 矩陣（需先 build 各 harness）
