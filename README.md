...
- **Cross-language & cross-platform interop**: standard `$argon2id$v=19$…` PHC format; a hash produced by .NET verifies under Node/Python/PHP/Edge and vice versa. A 4×4 cross-language round-trip matrix gates merges; Edge is proven bit-identical against the same frozen vectors via its own workerd conformance job.
...
.github/workflows/  per-language CI, guard 1 (spec invariants), guard 3 (4×4 matrix), edge (frozen-vector conformance + workerd), nightly, release
```
...
- **跨語言與跨平台互通**：`$argon2id$v=19$…` 標準 PHC 格式；.NET 產的雜湊 Node／Python／PHP／Edge 都能驗，反之亦然。4×4 跨語言 round-trip 矩陣擋 merge；Edge 以同一組凍結向量透過 workerd conformance 證明 bit-identical。
...
.github/workflows/  per-language CI、守門 1（spec 不變式）、守門 3（4×4 矩陣）、edge（凍結向量 conformance + workerd）、nightly、release
```
